...
namespace Ctrl {
    ...
    public class TileData_Maneger : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler
    {
        ...
        {
            //多分 !=Stage の部分がのちにバグる可能性あり
            if (collision.gameObject.tag != "Stage")
            {
                //ここで子オブジェクトに入れている
                //childObj = collision.gameObject;
                //collision.transform.parent = this.gameObject.transform;
                //myNumber = childObj.GetComponent<BldgData>().myNumber;

                {//結合時、個々のカッコ内は全て消す
                    //CardDataProxy SetCardData = new CardDataProxy(childObj.GetComponent<BldgData>().CardNumberEntity, childObj.GetComponent<BldgData>().CardNameEntity, childObj.GetComponent<BldgData>().CardTextEntity,
                    //                                        childObj.GetComponent<BldgData>().CardCostEntity, childObj.GetComponent<BldgData>().CardScoreEntity, childObj.GetComponent<BldgData>().CardTypeEntity, 0);
                    //_Field = new FieldDataProxy(SetCardData, 1, 0, false, -1);
                ...
                //Debug.Log(myNumber);
                //SearchRankUp(); //この中で廃業になるため、2回目のSearchRankUpは発生しない
            }
        }
