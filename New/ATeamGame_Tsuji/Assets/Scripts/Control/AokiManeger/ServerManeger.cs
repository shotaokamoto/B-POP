...
namespace Ctrl
{
    public class ServerManeger : NetworkBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (IsFarst)
            {
                if (DrawManager.instance.pNum == 0)
                {
                    //おジャマ岩配置
                    CreateOjamaRock();
                ...
                {
                    //gameObject.GetComponent<ServerManeger>().enabled = false;
                    print("NotHost");
                    IsFarst = false;
                }
        ...
        private void PlayerInitialize()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                _DrawCard[i] = new List<CardDataProxy>();
            }
        ...
        private void ConfirmScore()
        {
            ...
            for (int player = 0; player < 4; player++)
            {
                ...
                //マネージャに変更をかける
                _PlayerMgrObj.GetComponent<PlayerManeger>().Player[SortPlayer[player].PlayerNumber - 1] = SortPlayer[player];
            }
        }
        ...
        public void RpcRecvProssess(SendSeverDataArray[] sendData)
        {
            ...
            //if (_PlayerObj[this.gameObject.GetComponent<LocalPlayerManeger>().playerNum] != null)
            //{
            //    _PlayerObj[this.gameObject.GetComponent<LocalPlayerManeger>().playerNum].GetComponent<LocalPlayerManeger>().recvSeverData = sendData[this.gameObject.GetComponent<LocalPlayerManeger>().playerNum];       //各プレイヤーの受け取るデータ構造体にデータを挿入
            //    _PlayerObj[this.gameObject.GetComponent<LocalPlayerManeger>().playerNum].GetComponent<LocalPlayerManeger>().recvSeverData.isRecvFinished = true;
            //}

            if (_PlayerObj[PlayerNum] != null)
            {
                _PlayerObj[PlayerNum].GetComponent<LocalPlayerManeger>().recvSeverData = sendData[PlayerNum];       //各プレイヤーの受け取るデータ構造体にデータを挿入
                _PlayerObj[PlayerNum].GetComponent<LocalPlayerManeger>().recvSeverData.isRecvFinished = true;
            }
