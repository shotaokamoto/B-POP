...
public class NetworkMangerCustom : NetworkManager
{
    ...
    {
        int teamIndex = GameMgr.GetInstance().GetTeamFill();

        Vector3 startPos = GameMgr.GetInstance().GetSpawnPosition(teamIndex);
        ...
        GameMgr.GetInstance().size[teamIndex]++;
        GameMgr.GetInstance().UI.OnPlayerSizeChanged(SyncListInt.Operation.OP_DIRTY, teamIndex);
